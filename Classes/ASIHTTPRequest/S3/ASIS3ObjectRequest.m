...
+ (id)requestWithBucket:(NSString *)bucket key:(NSString *)key
{
	...
	ASIS3ObjectRequest *request = [[(ASIS3ObjectRequest*)[self alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@.s3.amazonaws.com%@",bucket,path]]] autorelease];
...
{
	NSString *path = [ASIS3Request stringByURLEncodingForS3Path:key];
	ASIS3ObjectRequest *request = [[(ASIS3ObjectRequest*)[self alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@.s3.amazonaws.com%@?%@",bucket,path,subResource]]] autorelease];
