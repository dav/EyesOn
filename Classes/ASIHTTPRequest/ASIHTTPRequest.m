...
NSString *ASIHTTPRequestVersion = @"v1.7-63 2010-10-13";
...
{
	return [[(ASIHTTPRequest*)[self alloc] initWithURL:newURL] autorelease];
}
...
{
	ASIHTTPRequest *request = [[(ASIHTTPRequest*)[self alloc] initWithURL:newURL] autorelease];
...
- (NSDictionary *)findSessionAuthenticationCredentials
{
	...
	for (NSDictionary *theCredentials in sessionCredentialsList) {
		if ([(NSString*)[theCredentials objectForKey:@"URL"] isEqual:[self url]]) {
